...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!storeUrlElement || !currentUserIdElement || !currentTaskIdElement) {
    ...
    const currentTaskId = parseInt(currentTaskIdElement.dataset.taskId);
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            console.log('フォーム送信:', {
                action: form.action,
                method: form.method,
                isStoreUrl: isStoreUrl,
                hasMethodField: !!document.getElementById('formMethod'),
    ...
    window.openNewTimeEntryModal = function() {
        openNewTimeEntryModal(storeUrl, currentTaskId, todayDate);
    };
});

// 新規登録モーダルを開く
function openNewTimeEntryModal(storeUrl, currentTaskId, todayDate) {
    const modal = getTimeEntryModal();
    if (!modal) {
    ...
    if (taskIdSelect) taskIdSelect.value = currentTaskId;
